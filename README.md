...
- **多 Key（一行一个）**：API Key 支持填多个，换行分隔（逗号/分号亦可）。填 N 个会自动拆成 N 条渠道，名称自动加 `-1 ~ -N`，共享同一份模型/优先级/权重配置，请求在它们之间**严格轮询**——适合多账号分摊额度，某个 Key 限流或失效时自动跳到下一个
- **按组折叠**：多 Key 拆分出的渠道归为一组，列表默认折叠成一行，显示 `几/几 可用`、合计请求与失败、平均延迟；展开可单独管理每一条。组行支持「改配置」（一次性同步全组的模型/BaseURL/优先级/权重）、整组启停、整组删除
- **从上游获取模型**：填好 URL + Key 后点一下，实时探测上游 `/v1/models` 填充下拉，可点选可手输（填 `*` 通配所有模型）
- **模型测试**：一行点「模型测试」打开弹窗——默认选中该渠道的全部模型（含别名映射），可增删（支持手动输入模型名），点「开始测试」逐条真实调用（`max_tokens=16`，上游有下限要求时自动换 64 重试），结果展示模型 / 上游实际名 / 可用状态 / 延迟 / 失败原因，可勾选「只看失败」。测试不写入熔断统计，失败不会停用渠道。收费模型会产生极少量 token 消耗，请按需选择。
